...
if (key==null || keyword==null || key.equals("null") || keyword.equals("null")){
	strSQL = "SELECT MAX(num) FROM free_free6";	//tblboard의 모든 글을 카운트 한다.
}else{
	strSQL = "SELECT count(*) FROM free_free6 WHERE " + key + " like '%" + keyword + "%'";
}
...
<center><font size='3'><b> 자유 게시판 </b></font></TD>
...
if(lastRow > 0) {	//글의 개수가 0보다 크면 글 보여주기
	if(key==null || keyword==null || key.equals("null") || keyword.equals("null")){
		strSQL = "SELECT * FROM free_free6 WHERE num BETWEEN " + startRow + " and " + endRow + " ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	} else {
		strSQL = "SELECT * FROM free_free6 WHERE " + key + " like '%" + keyword + "%' ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	}
...
%>
	...
	<TR bgcolor='ededed'>     
		...
		<TD align=left valign="middle">
			<a href="main.jsp?menu=free&subMenu=free&action=read&num=<%=listnum %>">
...
%>
		<a href="main.jsp?menu=free&subMenu=free&pageNum=<%=currentPage-1%>">[이전]</a>	
<%	
	}
...
%>
		<a href="main.jsp?menu=free&subMenu=free&pageNum=<%=i%>">[<%=i%>]</a>
<%
		}
	}
	if(lastPage > currentPage) {	//현재 페이지가 마지막 페이지보다 작을 경우에만
%>
		<a href="main.jsp?menu=free&subMenu=free&pageNum=<%=currentPage+1%>">[다음]</a>
<%
	}
}
%>  
...
<TABLE border=0 width=600>
	<TR>
		<TD align='center'>	
			...
			<input type='hidden' name='menu' value='free'>
			<input type='hidden' name='subMenu' value='free'>
			<TR>
				<TD align='right'>
...
<%if(session.getAttribute("LOGIN").equals("YES")) { %>
		<a href="main.jsp?menu=free&subMenu=free&action=write">[등록]</a>
<%} else { %>
		[등록]
<%} %>
		</TD>

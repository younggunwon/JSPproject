...
if (key==null || keyword==null || key.equals("null") || keyword.equals("null")){
	strSQL = "SELECT MAX(num) FROM noti_event6";	//tblboard의 모든 글을 카운트 한다.
}else{
	strSQL = "SELECT count(*) FROM noti_event6 WHERE " + key + " like '%" + keyword + "%'";
}
...
if(lastRow > 0) {	//글의 개수가 0보다 크면 글 보여주기
	if(key==null || keyword==null || key.equals("null") || keyword.equals("null")){
		strSQL = "SELECT * FROM noti_event6 WHERE num BETWEEN " + startRow + " and " + endRow + " ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	} else {
		strSQL = "SELECT * FROM noti_event6 WHERE " + key + " like '%" + keyword + "%' ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	}
...
%>
			<a href="main.jsp?menu=noti&subMenu=event&action=read&num=<%=listnum %>">
<% 
			}
%>
...
%>
		<a href="main.jsp?menu=noti&subMenu=event&pageNum=<%=currentPage-1%>">[이전]</a>	
<%	
	}
...
%>
		<a href="main.jsp?menu=noti&subMenu=event&pageNum=<%=i%>">[<%=i%>]</a>
<%
		}
	}
	if(lastPage > currentPage) {	//현재 페이지가 마지막 페이지보다 작을 경우에만
%>
		<a href="main.jsp?menu=noti&subMenu=event&pageNum=<%=currentPage+1%>">[다음]</a>
<%
	}
}
%>  
...
<TABLE border=0 width=600>
	<TR>
		<TD align='center'>	
			...
			<input type='hidden' name='subMenu' value='event'>
			<TR>
				<TD align='right'>
		...
		<TD align='right'>
<%if(session.getAttribute("LOGIN").equals("YES")) { %>
		<a href="main.jsp?menu=noti&subMenu=event&action=write">[등록]</a>
<%} else { %>
		[등록]
<%} %>
		</TD>

...
String strSQL = "SELECT * FROM noti_job6 WHERE num = ?";
...
<TABLE border='0' width='600'>
	<TR>
		...
		<%
			if(myId.equals(id)) {
		%>
			<a href="main.jsp?menu=noti&subMenu=job&action=reset&num=<%=num %>">[수정하기]</a>
			<a href="main.jsp?menu=noti&subMenu=job&action=delete&num=<%=num %>">[삭제하기]</a>
		<%
			} else {
		%>
			[수정하기]
		...
		<TD align='right'>
			<a href="main.jsp?menu=noti&subMenu=job">[목록보기]</a>
 		</TD>
	</TR>
</TABLE>

<%
strSQL = "UPDATE noti_job6 SET readcount=readcount+1 WHERE num = ?";
pstmt = conn.prepareStatement(strSQL);
pstmt.setInt(1, Integer.parseInt(num));
pstmt.executeUpdate();
...
<FORM Name='Plus' Action='menu/job/noti_job_comment_input.jsp' Method='post'>
...
String strSQL = "SELECT * FROM noti_job_comment6 WHERE id = '" + num + "'";
...
%>
				<FORM Name='reset' Action='menu/job/noti_job_comment_reset.jsp' Method='post'>
...
%>
		...
		%>
		 	<a href="main.jsp?menu=noti&subMenu=job&action=read&num=<%=num %>&subAction=comment_reset&commentNum=<%=getCommentNum %>">[수정하기]</a>
		 	<a href="menu/job/noti_job_comment_delete.jsp?commentNum=<%=getCommentNum %>&num=<%=num %>">[삭제하기]</a>
		<%
			} else {
		%>
			[수정하기]
